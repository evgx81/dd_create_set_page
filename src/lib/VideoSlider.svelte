<script>
    ...
    import { render_in_progress, render_task_result_data } from "./stores";

    /**
     * Признак, того что видео готово к проигрыванию в видеоплеере
     * @type {boolean}
     */
    ...
    afterUpdate(() => {
        // Если начался процесс рендеринга, то отмечаем, что видео еще не готово к проигрыванию
        if (
            $render_in_progress &&
            $render_task_result_data.videos.length === 0
        ) {
            ready_to_play = false;
            video_can_be_loaded = false;
        }

        // Если в процессе рендеринга видео получено, но пока не загружено в элемент, то загружаем полученное видео в видеоплеер
        // и отмечаем, что оно загружено, чтобы избежать повторного обновления событий плеера
        // Такой механизм нужен для корректного отображения видео. Если его брать напрямую из стора, который
        // постоянно обновляется во время рендеринга, то все события плеера будут возникать повторно
        if (
            $render_task_result_data.videos.length > 0 &&
            video_can_be_loaded === false
        ) {
            curr_video_link = $render_task_result_data.videos[0];
            video_can_be_loaded = true;
        }
